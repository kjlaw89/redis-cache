...
To adjust the connection parameters, define any of the following constants in your `wp-config.php` file.
  ...
  * `WP_REDIS_TIMEOUT` (default: `5`)
	  ...
	  Amount of time in seconds to attempt initial connection to Redis server before failing.

  * `WP_REDIS_READ_TIMEOUT` (default: _not set_)

	  Amount of time in seconds to attempt a read from the Redis server before failing.

  * `WP_REDIS_RETRY_INTERVAL` (default: `0`)
